...
const roleStuff = () => {
  inquirer
    ...
    .then((response) => {
      connect.query(`INSERT role.id, title, salary, department.name AS department
      FROM role INNER JOIN department ON role.department_id = department.id` [response.roleName]);
